...
| `all` | `own` 加 53 个上游 Skill，共 62 个当前筛选后的可用 Skill | 想与作者当前筛选后的可用集合对齐 | 按每个上游项目和 Agent 的官方方式逐项安装 |
...
此档包含 `own`，并扩展到 [manifests/all-used.json](../manifests/all-used.json) 中记录的 53 个上游 Skill：9 个自有 Skill 加 53 个上游 Skill，共 62 个。它是一份有意筛选的可复现来源索引，不是扫描所有目录后得到的机械并集；工作区辅助目录、上游占位模板和仍在观察中的 Skill 不计入清单。
...
以下目录或模板被明确排除，不计入 53 个可加载上游 Skill：
